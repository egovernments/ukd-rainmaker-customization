...
//			tenantList = getTenantList();
	...
	private Map<String, String> rollOverForFailedProps(RequestInfoWrapper requestInfoWrapper,
		...
		List<Map<String, Object>> currentFinYear = masters.stream().filter(master -> master.get("code").equals(rollOverNextFinYear)).collect(Collectors.toList());
		List<Map<String, Object>> previousFinYear = masters.stream().filter(master -> master.get("code").equals(rollOverCurrFinYear)).collect(Collectors.toList());
		...
		for (Map<String, Object> prop : failedProps) {
			...
			if (res.getDemands().size() == 0) {
				rollOverRepository.saveRollOver(prop.get("propertyid").toString(), prop.get("tenantid").toString(),
						rollOverNextFinYear, "NOTINITIATED", "No Demands found this property ");
			} else {
					...
					if(CollectionUtils.isEmpty(prevDemand)){
						...
						try {
							...
							rollOverRepository.saveRollOver(prop.get("propertyid").toString(),
									prop.get("tenantid").toString(), rollOverNextFinYear, "SUCCESS",
							...
							rollOverRepository.saveRollOver(prop.get("propertyid").toString(),
									prop.get("tenantid").toString(), rollOverNextFinYear, "FAILED",
									"Assessment or Demand Creation Failed");
							responseMap.put(prop.get("propertyid").toString(), "Failure");
						}
					} else {
						rollOverRepository.saveRollOver(prop.get("propertyid").toString(),
								prop.get("tenantid").toString(), rollOverNextFinYear, "NOTINITIATED",
								"No Demands found this property ");
					}
				}
				else if (currDemand.get(0) != null) {
					rollOverRepository.saveRollOver(prop.get("propertyid").toString(), prop.get("tenantid").toString(),
							rollOverNextFinYear, "SUCCESS", "Roll Over is Successfully Done");
					responseMap.put(prop.get("propertyid").toString(), "Success");
				} 
			}
		}
			...
			for (Property property : properties) {
				...
				if (!CollectionUtils.isEmpty(res.getDemands())) {
					...
					try{
						...
						rollOverRepository.saveRollOver(property.getPropertyId(), property.getTenantId(), rollOverNextFinYear, "SUCCESS",
								"Roll Over is Successfully Done");
					}
					catch(Exception e)
					{						
						log.error("Assessment Creation Failed "+e.toString());
						rollOverRepository.saveRollOver(property.getPropertyId(), property.getTenantId(), rollOverNextFinYear, "FAILED",
								"Assessment or Demand Creation Failed");
					}
					}else{
						rollOverRepository.saveRollOver(property.getPropertyId(), property.getTenantId(), rollOverNextFinYear, "SUCCESS",
					...
					rollOverRepository.saveRollOver(property.getPropertyId(), property.getTenantId(), rollOverNextFinYear, "NOTINITIATED",
	...
	private Assessment createAssessmentForRollOver(List<Demand> newDemands, RequestInfo requestInfo, String tenantId,String propertyId) {
		Assessment assessment = Assessment.builder().assessmentDate(ZonedDateTime.now().toInstant().toEpochMilli()).financialYear(rollOverNextFinYear).channel(Channel.CFC_COUNTER).source(Source.LEGACY_RECORD).tenantId(tenantId).propertyId(propertyId).build();
	...
	private List<Demand> prepareDemandRequest(Demand demand, List<Map<String, Object>> finYears) {
		...
		Map<String, Object> financialYear = finYears.stream().filter(finYear -> finYear.get("code").equals(rollOverNextFinYear)).collect(Collectors.toList()).get(0);		
